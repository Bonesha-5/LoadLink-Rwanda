...
import { approveCompany, ensureSeedAdminData, getPendingCompanies, rejectCompany, getPendingTrucksDemo, approvePendingTruck, rejectPendingTruck, writeAuditLog } from '../data/storage'
...
// Seed trucks always visible in demo so admin can test the flow
...
export default function AdminCompanyVerification() {
  ...
  async function loadTrucks() {
    ...
    try {
      if (token) {
        ...
        // Demo: merge live submitted trucks with seed trucks
        const live = getPendingTrucksDemo().map(t => ({
          id: t.id,
          plateNumber: t.plateNumber,
          truckType: t.truckType,
          declaredCapacity: Number(t.capacity) || 0,
          companyName: t.companyName,
          companyId: t.companyEmail,
          insuranceCertPath: t.insuranceCertName ?? null,
          createdAt: t.createdAt,
        }))
        // Show live trucks first, then seeds (filter out seeds if same id already exists)
        const liveIds = new Set(live.map(t => t.id))
        const seeds = DEMO_PENDING_TRUCKS.filter(t => !liveIds.has(t.id))
        setTrucks([...live, ...seeds])
      }
  ...
  async function approveCompanyAction(id: string) {
    try {
      ...
      if (token) await approveCompanyApi(token, id)
        ...
        approveCompany(id, adminName)
        writeAuditLog(adminName, 'COMPANY_APPROVED', 'company', company?.name ?? id)
      }
  ...
  async function confirmRejectCompany(e: React.FormEvent) {
    ...
    try {
      ...
      const company = companies.find(c => c.id === rejectingId)
      if (!token) writeAuditLog(adminName, 'COMPANY_REJECTED', 'company', company?.name ?? rejectingId ?? '')
  ...
  async function approveTruck(id: string) {
    try {
      ...
      if (token) {
        ...
        // Try to approve in live queue (works for real submitted trucks)
        // For seed demo trucks, approvePendingTruck is a no-op which is fine
        approvePendingTruck(id)
        writeAuditLog(adminName, 'TRUCK_APPROVED', 'company', truck?.plateNumber ?? id)
      }
  ...
  async function confirmRejectTruck(e: React.FormEvent) {
    ...
    try {
      ...
      if (!token) rejectPendingTruck(rejectingTruckId) // no-op for seed trucks, works for real ones
