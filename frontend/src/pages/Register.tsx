...
export default function Register() {
  ...
  const isLoggedInForThisRole =
    user?.role === role && (role === "shipper" || hasToken);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (role === "admin") {
      setError(
        "Admin accounts are not self-registered. Use the admin sign-in page with credentials provided by your team.",
      );
    ...
    if (role === "company") {
      ...
      if (
        ...
        setError(
          "Fill in RDB number, contact person, phone, and base district.",
        );
        return;
      }
      setBusy(true);
        ...
        if (isMockAuthMode()) {
          ...
          navigate("/login/company", {
            ...
            state: {
              registered: true,
              registeredMessage:
                "Demo company registered and queued for approval. You can sign in now; the admin can approve you in “Company Verification”.",
            },
          });
        } else {
          await companyRegister({
            ...
            business_cert_url: businessCertFile?.name ?? "",
            insurance_doc_url: insuranceDocFile?.name ?? "",
          });
          navigate("/login/company", {
            replace: true,
            state: {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              placeholder={
                role === "company" ? "e.g. company@example.com" : "e.g. alice"
              }
              value={username}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <label
                  ...
                  className="flex items-center gap-3 w-full px-4 py-3.5 rounded-xl bg-stone-50 border border-stone-200 text-stone-800 cursor-pointer hover:bg-stone-100 focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/20 transition-all"
                >
                  <svg className="w-4 h-4 text-stone-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                  </svg>
                  <span className={`text-sm truncate ${businessCertFile ? "text-stone-800" : "text-stone-400"}`}>
                    {businessCertFile ? businessCertFile.name : "Upload PDF or image"}
                  </span>
                  <input
                    id="businessCertFile"
                    type="file"
              ...
              <div className="sm:col-span-2">
                ...
                >
                  ...
                  <span className={`text-sm truncate ${insuranceDocFile ? "text-stone-800" : "text-stone-400"}`}>
                    {insuranceDocFile ? insuranceDocFile.name : "Upload PDF or image"}
                  </span>
                  <input
                    id="insuranceDocFile"
                    type="file"
                    accept=".pdf,.jpg,.jpeg,.png"
                    className="sr-only"
                    onChange={(e) => setInsuranceDocFile(e.target.files?.[0] ?? null)}
                  />
                </label>
              </div>
            </div>
        ...
        >
          ...
          <div className="relative z-10 space-y-8">
            ...
            <p className="text-white/90 text-lg max-w-sm">
              Create your shipper account to post loads, receive offers from
              verified transport companies, and manage shipments in one place.
            </p>
          ...
          <p className="relative z-10 text-sm text-white/70">
            After registering you&apos;ll go to the home page signed in as a
            shipper (demo).
          </p>
        </div>
      ...
      <div className="min-h-screen flex">
        ...
        >
          ...
          <div className="relative z-10 space-y-8">
            ...
            <p className="text-white/90 text-lg max-w-sm">
              Register to receive shipment requests from shippers, submit
              offers, and manage your trucks on Rwanda&apos;s logistics
              marketplace.
            </p>
            <ul className="space-y-3">
          ...
          <p className="relative z-10 text-sm text-white/70">
            After a successful registration you&apos;ll go to company sign-in to
            use your account.
          </p>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-10 bg-[#F6F7FB] min-h-screen">
