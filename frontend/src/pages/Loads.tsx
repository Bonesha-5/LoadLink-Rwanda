...
export default function Loads() {
  ...
  // Re-read localStorage when window regains focus (e.g. after company expresses interest)
  useEffect(() => {
    const onFocus = () => setRefresh(v => v + 1)
    window.addEventListener('focus', onFocus)
    // Also poll every 3s while page is visible so updates show without needing to switch tabs
                ...
                >
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {stage === 'POSTED' && (l.offers?.length ?? 0) > 0 && (
                        <span className="inline-flex items-center gap-1 rounded-full bg-accent/10 border border-accent/20 px-2 py-0.5 text-xs font-bold text-accent">
                          {l.offers!.length} {l.offers!.length === 1 ? 'interest' : 'interests'}
                        </span>
                      )}
                    </div>
                  </td>
