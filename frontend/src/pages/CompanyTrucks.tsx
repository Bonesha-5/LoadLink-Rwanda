              ...
              <div className="flex justify-between items-start gap-2">
                ...
                <span
                  className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                    STATUS_STYLES[String(truck.availability_status ?? truck.availabilityStatus ?? '')] ??
                    'bg-stone-100 text-stone-500'
                  }`}
                >
                  {String(truck.availability_status ?? truck.availabilityStatus ?? 'AVAILABLE').replace(/_/g, ' ')}
                </span>
              </div>
            </li>
          ))}
