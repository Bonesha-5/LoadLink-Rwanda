...
import {
  ...
  getRatingsByTruck,
  addPendingTruck,
  getPendingTrucksDemo,
  getCompanyByEmailDemo,
  type PendingTruck,
} from '../data/storage'
...
export default function CompanyTrucks() {
  ...
  // Sync live from storage so admin approval works without re-login
  const companyEmail = user?.email ?? ''
  const liveStatus = (() => {
    if (!companyEmail) return user?.status ?? ''
    try {
      const c = getCompanyByEmailDemo(companyEmail)
      return c?.status ?? user?.status ?? ''
    } catch { return user?.status ?? '' }
  })()
  const isVerified = String(liveStatus).toUpperCase() === 'VERIFIED'
  ...
  const pendingTrucks = useMemo(() => {
    void refresh
    if (apiToken) return [] as PendingTruck[]
    return getPendingTrucksDemo().filter(t => t.companyName === companyName)
  }, [companyName, refresh, apiToken])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (apiToken) {
      ...
      try {
        await registerTruck(apiToken, {
          ...
          reg_card_path: insuranceCertFile?.name ?? 'uploaded/placeholder.pdf',
        })
        setCapacity('')
        setPlateNumber('')
    ...
    addPendingTruck({
      plateNumber: plateNumber.trim() || `RAB-${Math.floor(Math.random()*900+100)}-A`,
      truckType,
      capacity: `${capacity.trim()} tons`,
      companyName,
      companyEmail: email,
      insuranceCertName: insuranceCertFile?.name,
    })
    setCapacity('')
    setPlateNumber('')
    setTruckType('Flatbed')
  ...
  return (
    <div className="max-w-4xl">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-lg p-6 mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={() => { setShowForm(false); setCapacity(''); setPlateNumber(''); setTruckType('Flatbed'); setInsuranceCertFile(null) }}
      ...
      {/* Pending trucks — awaiting admin approval */}
      {!apiToken && pendingTrucks.length > 0 && (
        <div className="mb-6">
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {pendingTrucks.map((t) => (
              <li key={t.id} className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
                <span className="mt-0.5 w-2 h-2 rounded-full bg-amber-400 flex-shrink-0" />
                <div>
                  <p className="font-semibold text-stone-800">{t.plateNumber} · {t.truckType}</p>
                  <p className="text-sm text-stone-600">{t.capacity} · {t.companyName}</p>
                  <span className="mt-1.5 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-200">
              ...
              <div className="flex justify-between items-start gap-2">
                <div>
                  ...
                  <p className="text-stone-600 text-sm mt-1">
                    {apiToken ? `Type: ${truck.truck_type ?? truck.truckType ?? '—'}` : `Type: ${truck.type ?? '—'}`}
                  </p>
