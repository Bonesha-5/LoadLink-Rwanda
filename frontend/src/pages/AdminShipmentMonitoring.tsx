...
import { ensureSeedAdminData, getAdminShipments, getAllRealShipmentsForAdmin, type ShipmentStatus } from '../data/storage'
...
export default function AdminShipmentMonitoring() {
  ...
  async function loadShipments() {
    ...
    try {
      ...
      ensureSeedAdminData()
      // Merge seeded admin shipments with real shipper loads
      const realLoads = getAllRealShipmentsForAdmin()
      const seeded = getAdminShipments()
      // Prefer real loads; seed only if no real data yet
      const allShips = realLoads.length > 0 ? [...realLoads, ...seeded] : seeded
      const data = allShips.map((s): AdminShipment => ({
        id: s.id,
