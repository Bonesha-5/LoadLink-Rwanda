...
import { ensureSeedAdminData, getAdminDisputes, getAllRealShipmentsForAdmin, resolveDispute as resolveDisputeInStore, setStageForLoad } from '../data/storage'
...
export default function AdminDisputes() {
  ...
  async function loadDisputes() {
    ...
    try {
      ...
      ensureSeedAdminData()
      // Merge seeded disputes with real DISPUTED loads from shipper world
      const realDisputed = getAllRealShipmentsForAdmin().filter(s => s.status === 'DISPUTED')
      const seededDisputed = getAdminDisputes()
      const allDisputed = [...realDisputed, ...seededDisputed]
      const data = allDisputed.map((s): DisputedShipment => ({
        id: s.id,
  ...
  async function resolveDispute(e: React.FormEvent) {
    ...
    try {
      if (token) {
        ...
        // Also update ll_loads stage if this is a real shipper load
        try { setStageForLoad(currentDispute.id, 'COMPLETED') } catch { /* not in ll_loads */ }
      }
